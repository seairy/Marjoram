    ...
    def export_users users
      StringIO.new.tap do |file|
        ...
        users.each_with_index do |user, i|
          sheet.insert_row(i + 1, [user.id, user.human_type, user.human_state, user.chinese_name, user.foreign_name, user.account, gender_tag(user.gender), user.country.name, user.address, user.postal_code, user.phone_number, user.mobile_number, user.fax_number, user.email, user.organization, user.professional_title, user.position, boolean_tag(user.is_member?), user.subject, user.first_author, user.second_author, (user.group_a_review ? user.group_a_review.expert.name : '未指派'), (user.group_a_review ? user.group_a_review.human_result : '未指派'), (user.group_b_review ? user.group_b_review.expert.name : '未指派'), (user.group_b_review ? user.group_b_review.human_result : '未指派'), user.keywords, user.outline, (user.file.blank? ? '未提交' : '已提交'), user.created_at.strftime('%Y-%m-%d %H:%M')])
        end
        book.write(file)
      end
    end
